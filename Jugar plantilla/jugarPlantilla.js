...
function generarPreguntas() {
  ...
  preguntas.forEach((pregunta, index) => {
    ...
    const tipoPregunta = parseInt(
      pregunta.ID_TIPO_PREGUNTA || pregunta.tipo_pregunta
    );
    ...
    card.innerHTML = `
            <div class="text-center pregunta">
                ...
                ${
                  pregunta.IMAGEN
                    ? `<img src="${pregunta.IMAGEN}" alt="Imagen de la pregunta" class="mb-3 pregunta_img">`
                    : ""
                }
            </div>
...
function actualizarCronometro() {
  ...
  cronometroEl.textContent = `${minutos.toString().padStart(2, "0")}:${segundos
    .toString()
    .padStart(2, "0")}`;
...
function manejarRespuestaUnica(card, onContinuar) {
  card.querySelectorAll(".btnRespuestas").forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      card
        .querySelectorAll(".btnRespuestas")
...
function manejarRespuestaAbierta(card, onContinuar) {
  ...
  btnEnviar.addEventListener("click", () => {
    ...
    if (!respuesta) {
      mostrarMensajeError("Por favor escribe una respuesta");
      return;
    }

    const idPregunta = parseInt(textarea.dataset.idPregunta);

    const pregunta = version.preguntas.find(
    ...
    try {
      ...
      if (invitado === "true" && codigoVersion) {
        response = await fetch(
          `../BaseDeDatos/controladores/getVersionByCode.php?codigo=${codigoVersion}&invitado=true`
        );
      } else {
        response = await fetch(
          `../BaseDeDatos/controladores/getVersionById.php?version=${idVersion}&jugador=true`
        );
      }
        ...
        if (version.ACTIVO === "Inactivo") {
          ...
          if (version.IMAGEN) {
            document.getElementById(
              "contenedor-juego"
            ).style.backgroundImage = `linear-gradient(rgba(0, 0, 0, 0.6), rgba(0, 0, 0, 0.6)), url('${version.IMAGEN}')`;
          } else {
            document.getElementById(
              "contenedor-juego"
            ).style.backgroundImage = `linear-gradient(rgba(0, 0, 0, 0.6), rgba(0, 0, 0, 0.6)), url('../images/fondo_default.jpg')`;
          }
          generarPreguntas();
        }
      } else if (result.status === "error") {
        redirigirConError(
          result.message || "Error al obtener el cuestionario",
          invitado
        );
      }
    } catch (error) {
  ...
  try {
    ...
    if (invitado) {
      const nombreInvitado =
        sessionStorage.getItem("nombreInvitado") || "Invitado";
      response = await fetch(
        "../BaseDeDatos/controladores/postParticipacionInvitado.php",
        {
          method: "POST",
          headers: {
      ...
      const invitacion = new URLSearchParams(window.location.search).get(
        "invitacion"
      );
      response = await fetch(
        "../BaseDeDatos/controladores/postParticipacion.php",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
    ...
    if (result.status === "success") {
      ...
      redirigirConError(
        result.message || "Error al guardar participación",
        invitado
      );
    }
  } catch (error) {
